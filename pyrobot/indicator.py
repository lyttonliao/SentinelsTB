...
class Indicators():
    ...
    def rsi(self, period: int, method: str = 'wilders') -> pd.DataFrame:
        ...
        # Define the exponentially weighted moving average up days
        self._frame['ewma_up'] = self._price_groups['up_day'].transform(
            lambda x: x.ewm(span=period).mean()
        )

        # Define the exponentially weighted moving average down days
        ...
        self._frame['rsi'] = np.where(relative_strength_index == 0, 100, 100 - ( 100 / (1.0 + relative_strength)))

        # Clean up before sending back (deleting the column)
        self._frame.drop(
